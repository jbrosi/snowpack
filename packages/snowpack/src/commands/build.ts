...
export async function command(commandOptions: CommandOptions) {
  ...
  function logEvt({level, id, msg, args}) {
  ...
  messageBus.on('CONSOLE', logEvt);
  messageBus.on('WORKER_COMPLETE', logEvt);
  messageBus.on('WORKER_MSG', logEvt);
  messageBus.on('WORKER_RESET', logEvt);
  messageBus.on('WORKER_START', logEvt);
  messageBus.on('WORKER_UPDATE', logEvt);
  ...
  if (config.buildOptions.minify) {
    ...
    console.log(colors.yellow('! optimizing for production...'));
    ...
    console.log(
      `${colors.green('✔')} ${colors.bold('snowpack')} optimization complete ${colors.dim(
